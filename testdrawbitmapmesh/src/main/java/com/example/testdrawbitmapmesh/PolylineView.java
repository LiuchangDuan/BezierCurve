...
public class PolylineView extends View {
    ...
    private static final float LEFT = 1 / 16F, TOP = 1 / 16F,
            RIGHT = 15 / 16F, BOTTOM = 7 / 8F;

    // 文字坐标相对位置
    private static final float TIME_X = 3 / 32F, TIME_Y = 1 / 16F,
            MONEY_X = 31 / 32F, MONEY_Y = 15 / 16F;
    ...
    private void drawLines(Canvas canvas) {
        ...
        // 锁定画布并设置画布透明度为75%
        int sc = canvas.saveLayerAlpha(0, 0, canvas.getWidth(), canvas.getHeight(),
                75, Canvas.ALL_SAVE_FLAG);
        ...
        for (float y = viewSize * BOTTOM - spaceY; y > viewSize * TOP; y -= spaceY) {
            for (float x = viewSize * LEFT; x < viewSize * RIGHT; x += spaceX) {
                ...
                if (y == viewSize * BOTTOM - spaceY) {
                    canvas.drawText(String.valueOf(rulerX[index_x]), x,
                            y + textSignSize + spaceY, mTextPaint);
                }
                // 绘制纵轴刻度数值
                if (x == viewSize * LEFT) {
                    canvas.drawText(String.valueOf(rulerY[index_y]), x - thickLineWidth,
                            y + textRulerSize, mTextPaint);
                }
                index_x++;
                if (index_x >= pointFs.size() / 2) {
                    index_x = 0;
                }
            }
            index_y++;
            if (index_y >= pointFs.size() / 2) {
                index_y = 0;
            }
        }
    ...
    /**
     * 绘制曲线
     * 这里使用一个新的Bitmap对象结合新的Canvas对象来绘制曲线
     * 当然你可以直接在原来的canvas（onDraw传来的那个）中直接绘制如果你还没被坐标搞晕的话……
     * @param canvas 画布
     */
    private void drawPolyline(Canvas canvas) {
        // 生成一个Bitmap对象大小和我们的网格大小一致
        mBitmap = Bitmap.createBitmap((int) (viewSize * (RIGHT - LEFT) - spaceX),
                (int) (viewSize * (BOTTOM - TOP) - spaceY), Bitmap.Config.ARGB_8888);
